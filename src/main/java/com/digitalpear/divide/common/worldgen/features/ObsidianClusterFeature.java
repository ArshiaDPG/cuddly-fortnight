...
public class ObsidianClusterFeature extends Feature<DripstoneClusterFeatureConfig> {

	public ObsidianClusterFeature(Codec<DripstoneClusterFeatureConfig> codec) {
		super(codec);
	}
	...
	public boolean place(FeatureContext<DripstoneClusterFeatureConfig> context) {
		...
		if (!canGenerate(structureWorldAccess, blockPos)) {
			...
			int i = dripstoneClusterFeatureConfig.height.get(randomGenerator);
			float f = dripstoneClusterFeatureConfig.wetness.get(randomGenerator);
			float g = dripstoneClusterFeatureConfig.density.get(randomGenerator);
			int j = dripstoneClusterFeatureConfig.radius.get(randomGenerator);
			int k = dripstoneClusterFeatureConfig.radius.get(randomGenerator);

			for(int l = -j; l <= j; ++l) {
				for(int m = -k; m <= k; ++m) {
					double d = this.dripstoneChance(j, k, l, m, dripstoneClusterFeatureConfig);
					BlockPos blockPos2 = blockPos.add(l, 0, m);
					this.generate(structureWorldAccess, randomGenerator, blockPos2, l, m, f, d, i, g, dripstoneClusterFeatureConfig);
				}
			}
	...
	private void generate(StructureWorldAccess world, RandomGenerator random, BlockPos pos, int localX, int localZ, float wetness, double dripstoneChance, int height, float density, DripstoneClusterFeatureConfig config) {
		Optional<CaveSurface> optional = CaveSurface.create(world, pos, config.floorToCeilingSearchRange, DripstoneHelper::canGenerate, DripstoneHelper::isNeitherEmptyNorWater);
		if (optional.isPresent()) {
			...
			if (optionalInt.isPresent() || optionalInt2.isPresent()) {
				...
				if (optionalInt.isPresent() && bl2 && !this.isLava(world, pos.withY(optionalInt.getAsInt()))) {
					...
					this.placeObsidianBlocks(world, pos.withY(optionalInt.getAsInt()), j, Direction.UP);
					int k;
				...
				if (optionalInt3.isPresent() && bl3 && !this.isLava(world, pos.withY(optionalInt3.getAsInt()))) {
					...
					this.placeObsidianBlocks(world, pos.withY(optionalInt3.getAsInt()), m, Direction.DOWN);
	...
	private boolean isStoneOrWater(WorldAccess world, BlockPos pos) {
		BlockState blockState = world.getBlockState(pos);
		return blockState.isIn(DivideBlockTags.BASE_STONE_DIVIDE) || blockState.getFluidState().isIn(FluidTags.WATER);
	}

	private void placeObsidianBlocks(StructureWorldAccess world, BlockPos pos, int height, Direction direction) {
		BlockPos.Mutable mutable = pos.mutableCopy();
		for(int i = 0; i < height; ++i) {
			if (!generateObsidianBlock(world, mutable)) {
				return;
			}
	...
	protected static boolean generateObsidianBlock(WorldAccess world, BlockPos pos) {
		BlockState blockState = world.getBlockState(pos);
		if (blockState.isIn(DivideBlockTags.BASE_STONE_DIVIDE)) {
			world.setBlockState(pos, world.getRandom().nextFloat() > 0.9 ? Blocks.OBSIDIAN.getDefaultState() : DivideBlocks.COLD_OBSIDIAN.getDefaultState(), 2);
			return true;
		} else {
			return false;
		}
	...
	private double dripstoneChance(int radiusX, int radiusZ, int localX, int localZ, DripstoneClusterFeatureConfig config) {
		...
		int k = Math.min(i, j);
		return (double)MathHelper.method_37958((float)k, 0.0F, (float)config.maxDistanceFromCenterAffectingChanceOfDripstoneColumn, config.chanceOfDripstoneColumnAtMaxDistanceFromCenter, 1.0F);
	}

	private static float clampedGaussian(RandomGenerator random, float min, float max, float mean, float deviation) {
		return ClampedNormalFloatProvider.m_tqtsfibo(random, mean, deviation, min, max);
	}
}
